...
class CharacterController():
    ...
    def update_state(self, 
        ...
        #TDOD 自己的代码
        # print("joint_position_outside", self.motions[0].joint_position[0])
        # print("joint_position_30", self.motions[0].joint_position[30])
        frame_position = self.motions[self.cur_seq].joint_position[[self.cur_frame, self.cur_frame+1]].copy()
        frame_position[:, 0, [0,2]] = desired_pos_list[0, [0,2]]
        frame_rotation = self.motions[self.cur_seq].joint_rotation[[self.cur_frame, self.cur_frame+1]].copy()
        #apply damping
        if self.dampingParams.need_damping:
            # print("self.dampingParams.pos_diff", self.dampingParams.pos_diff)
            # print("self.dampingParams.vel_diff", self.dampingParams.vel_diff)
            # print("self.dampingParams.rot_diff", self.dampingParams.rot_diff)
            # print("self.dampingParams.avel_diff", self.dampingParams.avel_diff)
            offset, vel = decay_spring_implicit_damping_pos(
                self.dampingParams.pos_diff, self.dampingParams.vel_diff, half_life, self.dampingParams.time
            )
            offset_rot, avel = decay_spring_implicit_damping_rot(
                self.dampingParams.rot_diff, self.dampingParams.avel_diff, half_life, self.dampingParams.time
            )
            frame_position += offset
            # print(offset)
            for i in range(2):
                frame_rotation[i] = (R.from_rotvec(offset_rot) * R.from_quat(frame_rotation[i])).as_quat() 
        ...
        if min_seq_id == self.cur_seq and abs(min_frame_id-self.cur_frame) < 10:
            ...
            joint_position_dst = self.motions[min_seq_id].joint_position[[min_frame_id, min_frame_id+1]].copy()
            joint_rotation_dst = self.motions[min_seq_id].joint_rotation[[min_frame_id, min_frame_id+1]].copy()
            self.dampingParams.pos_diff = (frame_position[0] - joint_position_dst[0])
            ...
            self.dampingParams.rot_diff = (R.from_quat(frame_rotation[0]) * R.from_quat(joint_rotation_dst[0].copy()).inv()).as_rotvec()
            self.dampingParams.avel_diff = avel_src[0] - avel_dst[0]
            # print("rot_diff ", rot_diff)
            # print("self.dampingParams.avel_diff ", self.dampingParams.avel_diff)
            # print("half_life ", half_life)
            #set new seq and frame
